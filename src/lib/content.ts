...
export function cityHubDescription(city: City): string {
  return (
    ...
    `переезде, помощь с соседями и охрана мероприятий от Spectrum Guard. Оставьте заявку.`
  );
}
...
export function cityHubAnswerFirst(city: City): string {
  return (
    ...
    `Оставьте заявку или напишите в Telegram — свяжемся, обсудим задачу, и её возьмёт в работу ` +
    `команда ${city.prep}.`
  );
}
