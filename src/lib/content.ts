...
export function cityHubDescription(city: City): string {
  return (
    ...
    `переезде, помощь с соседями, охрана мероприятий и перевозка ценных грузов от Spectrum Guard. ` +
    `Оставьте заявку.`
  );
}
...
export function cityHubAnswerFirst(city: City): string {
  return (
    ...
    `перевозка ценных грузов. Оставьте заявку или напишите в Telegram — ` +
